...
//SM4算法常量和函数实现（T-table优化版）
class SM4 {
private:
    static const uint32_t FK[4];
    static const uint32_t CK[32];
    uint32_t rk[32]; //轮密钥
    uint32_t T_table[256]; //T-table用于优化轮函数计算
    ...
    //预计算T-table
    void precompute_T_table() {
        for (int i = 0; i < 256; ++i) {
            uint8_t s = sbox((uint8_t)i);
            //将字节转换为32位值并应用线性变换L
            uint32_t val = (uint32_t)s << 24;  // 放在最高位字节
            val = L(val);
            T_table[i] = val;
        }
    }
