...
# 基于签名算法误用做poc验证：
基于签名算法误用的POC 验证是指通过实际代码或实验，验证当签名算法被错误使用时，是否会导致安全漏洞，如私钥泄露、签名被伪造等。
## 缺陷 1：泄露随机数k导致私钥d泄露

### 误用场景
- 签名时生成的随机数`k`被意外泄露（如日志记录、代码漏洞等）
...
### 误用场景
- 不同用户（私钥分别为`d₁`和`d₂`）签名时使用了相同的`k`

### 攻击原理
1. 已知用户1的私钥`d₁`和签名`(r₁, s₁)`，可推导`k`：
k = s₁·(1+d₁) + r₁·d₁ mod n
2. 再用`k`和用户2的签名`(r₂, s₂)`推导`d₂`：
d₂ = (k - s₂) · (s₂ + r₂)⁻¹ mod n

### POC 验证
1. 让两个用户用相同`k`签名
2. 已知`d₁`时若能恢复`d₂`，则证明漏洞存在

---

## 缺陷 4：同一d和k用于ECDSA和SM2

### 误用场景
- 同一私钥`d`和随机数`k`同时用于ECDSA和SM2签名

### 攻击原理
1. ECDSA签名公式：
s_ecdsa = k⁻¹·(e_ecdsa + r_ecdsa·d) mod n
可反推`k`：
k = (e_ecdsa + r_ecdsa·d) · s_ecdsa⁻¹ mod n
2. 将`k`代入SM2的签名公式，即可求解`d`（同缺陷1的推导）

### POC 验证
1. 用相同`d`和`k`生成两种签名
2. 若能从签名中恢复`d`，则证明漏洞存在


# 伪造中本聪的数字签名：
### 步骤
1. 首先计算出与正常签名验证相同的ZA和e值
2. 随机生成r和s' 值，然后通过椭圆签名验证的逆过程反推是否满足验证条件
3. 当找到满足R=r条件的r和s' 时，就得到了一个可以通过验证的签名

**但是理论上在不知道私钥的情况下，无法有效伪造SM2数字签名，基本无法成功**
